    ...
    value: function () {
      ...
      for (var i = 0; i < children.length; i++) {
        ...
        if (child instanceof AEntity) {
          childEntities.push(child as never);
        }
      }
      ...
      return function (newMixins, oldMixins) {
        ...
        for (i = 0; i < this.mixinEls.length; i++) {
          for (component in this.mixinEls[i].componentCache) {
            if (componentsUpdated.indexOf(component as never) === -1) {
              ...
              componentsUpdated.push(component as never);
            }
          }
        }
...
export const AEntity = registerElement('a-entity', {prototype: proto});;